// UCNLDrivers/uAux/uAuxBPPort.cs
using UCNLNMEA;
...
namespace UCNLDrivers.uAux
{
    ...
    public class uAuxBPPort : uAuxPort, IGNSSPort
    {
        #region Properties
        ...
        public override AuxSourceKind Kind => AuxSourceKind.GNSS;

        #endregion

        #region Constructor

        public uAuxBPPort(string id, BaudRate baudRate)
            : base(baudRate)
        {
            Id = id;
            PortDescription = "BP GNSS";
            IsLogIncoming = true;
            IsTryAlways = true;
            IsRawModeOnly = true;
        ...
        private static double CalculateCourseOverGround(double northSpeed, double eastSpeed)
        {
            double sog = Math.Sqrt(northSpeed * northSpeed + eastSpeed * eastSpeed);
            if (sog < 0.01) return double.NaN;

            double degrees = Math.Atan2(eastSpeed, northSpeed) * 180.0 / Math.PI;
        ...
        private void OnPacketProcessed(ParsedPacket packet)
        {
            ...
            IsValid = packet.IsFullyValid;

            GroundSpeed = Math.Sqrt(NSpeed * NSpeed + ESpeed * ESpeed);
            CourseOverGround = CalculateCourseOverGround(NSpeed, ESpeed);
